#Rotete DITECTNET label flap (up-down)
#Ditectnetの形式のラベルのテキストファイルが入ったフォルダを指定して、すべてのラベルをflap(上下反転)させた画像に対応するラベルにします。
...
rotationflap <- function(x1,imgx,imgy){
 x1 <- as.matrix(x1)
 labels <- x1[,1]
...
for(i in 1:length(infiles1)){
	...
	outfname <- paste(strsplit(infiles1[i],"\\.")[[1]][1],"_rflap.txt",sep=" ")
	result <- rotationflap(infile,imgx,imgy)
