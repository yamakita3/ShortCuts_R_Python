...
#Outliner2na
#
outliner2na <- function(dat1,band1=2,quant=c(0.05,0.95)){
 outliner2navec <-function(dat=dat1,band=band1,quant=c(0.05,0.95)){
 ...
 if(is.null(dim(x))==T){
 	outliner2navec(dat=dat1,band=band1,quant)
 }else{
 	apply(dat1,2,function(x) outliner2navec(dat=x,band=band1))
 }
 return(x)
}
...
#}
#apply(x,2,function(x) outliner2navec(dat=x,band=2))
